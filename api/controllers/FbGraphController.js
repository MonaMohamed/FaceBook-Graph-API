...
// function(err, result) {
		graph.get("/me/friends",{limit: 50,access_token: req.session.fbAccessToken}, function(err, result) {
			  console.log("My Friendlist = " + JSON.stringify(result)); // {
																		// image:
				//console.log("Access token =" +req.session.fbAccessToken.toString() );														// true,
																		// location:
								// "http://profile.ak.fb..." }
				res.view("myFriends", {
	...
	showPostDetail: function(req, res, next) {
		...
		graph.get("/" + req.param("postId"), function(err, result) {
			...
			graph.get("/" + req.param("postId") + "/comments", function(err, commentResult) {
				  ...
				  console.log(">>>>>>>>>>>>>> comments = " + JSON.stringify(commentResult)); // {
					...
					res.view("showPostDetail", {
						...
						title: "Post Detail",
						post: result,
						comments:(commentResult && commentResult.data) ? commentResult.data : [],
						//comments:(result.comments && result.comments.data) ? result.comments.data.toString() : [],
						likes: (result.likes && result.likes.data) ? result.likes.data : [],
						successMessage:""
				});
			});
/*			
